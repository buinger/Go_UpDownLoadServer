...
func getRootFolderName(r *http.Request) string {
	// 从查询参数中获取密码
	rootFolderName := r.URL.Query().Get("rootFolder")
	fmt.Println(rootFolderName)
	// 比对密码
...
func uploadFile(w http.ResponseWriter, r *http.Request) {
	...
	for i := 2; i < len(parts); i++ {
		folderName += ("/" + parts[i])
		if parts[i] == "" {
			http.Error(w, "文件夹和文件名称不能为空", http.StatusBadRequest)
			return
		}
	}
	...
	dataRootDir := filepath.Join(currentDir, "Data")
	// 创建目标文件夹路径
	dstFolder := (dataRootDir + folderName)
	...
	// 写入响应正文
	//w.WriteHeader(http.StatusOK) // 可省略，默认是 200 状态码
...
func downloadFile(w http.ResponseWriter, r *http.Request) {
	...
	// 设置文件路径
	filePath := currentDir + "/Data" + filePathTail
